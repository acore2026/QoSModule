...
func TestReliableQoSRequestReusesCachedRANResponse(t *testing.T) {
	ranCalls := 0
	ran := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ranCalls++
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"request_id":"req-reliable-qos","status":"ACCEPTED"}`))
	}))
	defer ran.Close()
	handler, err := NewQoSHandler(QoSConfig{
		RANEndpoint: ran.URL + ranapi.DefaultPath,
	...
	reliablePayload, err := encodeReliableRequest("transport-retry-1", qosPayload)
	if err != nil {
	...
	first := readReliableQoSResponse(t, conn, reliablePayload)
	second := readReliableQoSResponse(t, conn, reliablePayload)
	if string(first) != `{"request_id":"req-reliable-qos","status":"ACCEPTED"}` {
		t.Fatalf("first reliable response payload = %s", first)
	}
	if string(second) != string(first) {
		t.Fatalf("second reliable response payload = %s, want %s", second, first)
	}
	if ranCalls != 1 {
		t.Fatalf("RAN calls = %d, want 1", ranCalls)
	}
}

func readReliableQoSResponse(t *testing.T, conn *net.UDPConn, payload []byte) []byte {
	t.Helper()
	if _, err := conn.Write(payload); err != nil {
		t.Fatalf("write reliable QoS request: %v", err)
	}
	_ = conn.SetReadDeadline(time.Now().Add(time.Second))
	response := make([]byte, 1024)
	n, err := conn.Read(response)
	if err != nil {
		t.Fatalf("read reliable QoS response: %v", err)
	}
	_, responsePayload, ok := decodeReliableResponse(response[:n])
	if !ok {
		t.Fatalf("response is not reliable envelope: %s", response[:n])
	}
